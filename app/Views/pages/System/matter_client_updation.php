...
<div class="pagetitle w-100 float-start border-bottom pb-1">
  <h1 class="col-md-8 float-start">Client Updation for a matter</h1>
</div><!-- End Page Title -->
<section class="section dashboard">
<form method="post" action="" name="matterClientUpdation" id="matterClientUpdation" >
      <div class="row">
			...
			<div class="frms-sec d-inline-block w-100 bg-white p-3">				
				...
				<div class="d-inline-block w-100">
					<div class="col-md-2 float-start px-2 position-relative mb-3">
						<label class="d-inline-block w-100 mb-2 lbl-mn">Matter Code (Old)</label>
						<input type="text" name="old_matter_code" id="oldMatterCode"class="form-control">
						<i class="fa-solid fa-binoculars icn-vw" id="clientBinocular" onclick="showData('matter_code', '<?= '4207' ?>', 'oldMatterCode', [ 'oldMatterCode','oldmatterDesc','oldClientCode','oldClientName'], ['matter_code','matter_desc','client_code','client_name'],'matter_code')"  data-toggle="modal" data-target="#lookup" style="display:<?php if($option=='View'){ echo $redv;} ?>"></i>
					</div>
				...
				<div class="d-inline-block w-100">
					<div class="col-md-2 float-start px-2 position-relative mb-3">
						<label class="d-inline-block w-100 mb-2 lbl-mn">Client (New)</label>
						<input type="text" name="new_client_code"  id="newClientCode"  class="form-control">
						<i class="fa-solid fa-binoculars icn-vw" id="clientBinocular" onclick="showData('client_code', '<?= '4210' ?>', 'newClientCode', [ 'newClientCode','newClientName'], ['client_code','client_name'],'client_code')"  data-toggle="modal" data-target="#lookup" style="display:<?php if($option=='View'){ echo $redv;} ?>"></i>
					</div>
				...
				<input type="hidden" name="option" id="option" value="update">
                <input id="save_button" class="btn btn-primary cstmBtn mt-2" type="button" name="button" value="Confirm" onClick="return checkMatterClientUpdationdata()">
