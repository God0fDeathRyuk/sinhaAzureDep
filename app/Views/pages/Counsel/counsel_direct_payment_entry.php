      ...
      <div class="row">
		  <div class="col-md-12 mt-3">
			<div class="frms-sec d-inline-block w-100 bg-white p-3">
				...
				<div class="frms-sec-insde d-block float-start col-md-4 px-2 mb-4">
					...
					<input type="text" class="form-control readonly" id="entryDate" name="entry_date" value="<?= ($user_option != 'Add') ? date_conv($hdr_row['entry_date']) : date('d-m-Y') ?>"  onBlur="chkEntryDate(this)" required/>
				</div>
				<hr/>
				<div class="frms-sec-insde d-block float-start col-md-6 px-2 mb-4">
					<label class="d-inline-block w-100 mb-2 lbl-mn">Counsel <strong class="text-danger">*</strong></label>
                    <div class="position-relative Inpt-Vw-icn">
						...
						<?php } else { ?>
							<input type="text" class="form-control smlBxLft float-start w-100" name="associate_code" value="<?= $hdr_row['counsel_code'] ?>" <?= ($user_option == 'View' || $user_option == 'Delete') ? 'readonly' : '' ?> required />
						<?php } ?>
					</div>
				...
				<div class="frms-sec-insde d-block float-start col-md-6 px-2 mb-4">
					...
					<div class="position-relative Inpt-Vw-icn">
                        <input type="text" class="form-control smlBxLft float-start w-100" name="clerk_code" id="clerkCode" oninput="this.value = this.value.toUpperCase()" onchange="fetchData(this, 'associate_code', ['clerkName'], ['associate_name'], 'get_clerk_result')" value="<?= ($user_option == 'Add') ? '' : $hdr_row['clerk_code'] ?>" <?= ($user_option == 'View' || $user_option == 'Delete') ? 'readonly' : '' ?>/>
					<?php if ($user_option == 'Add' || $user_option == 'Select') { ?>
				...
				<div class="frms-sec-insde d-block float-start col-md-6 px-2 mb-4">
					...
					<?php } else { ?>
                        <input type="text" class="form-control smlBxLft float-start w-100" name="peon_code" value="<?= $hdr_row['peon_code'] ?>" readonly/>
					<?php } ?>
                    </div>
		  ...
		  <div class="col-md-12 mt-3">
			<span id="actionBtn1">
		 	 ...
		 	 <div class="tblMn mb-2 tblscrlvtrclExtlrg">
				<table class="table border-0">
					<tr class="fs-14">
						<th class="border w-3"></th>
						<th class="border w-8">Brief Date</th>
						<th class="border w-8">Memo No.</th>
						<th class="border w-8">Memo Dt.</th>
						<th class="border w-150">Matter.</th>
					...
					<tbody id="tbody">
						...
						?>
						<tr class="fs-14">
							<td id="Ctd<?= $i?>" class="border fw-normal align-middle text-center" onClick="<?php if($user_option=='Add'||$user_option=='Select'){ ?>voucher_delRow(this, <?= $i ?>)<?php }?>">
							...
							<td class="border fw-normal"> <input type="text" class="form-control" name="brief_date<?= $i?>" value="<?= ($row_1) ? date_conv($row_1['brief_date']) : '' ?>"  onBlur="chkBriefDate(this,'<?= $i ?>')" <?= $redvc ?> ></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="memo_no<?= $i?>" value="<?= ($row_1) ? $row_1['memo_no'] : ''?>" <?= $redvc ?> ></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="memo_date<?= $i?>" value="<?= ($row_1) ? date_conv($row_1['memo_date']) : ''?>" onBlur="chkMemoDate(this,'<?= $i ?>')" <?= $redvc ?> ></td>
							<td class="border fw-normal position-relative">
								<input type="text" class="form-control" name="matter_code<?= $i?>" id="matterCode<?= $i ?>" value="<?= ($row_1) ? $row_1['matter_code'] : ''?>" 	oninput="this.value = this.value.toUpperCase()" onchange="fetchData(this, 'matter_code', ['clientCode<?= $i ?>', 'initialCode<?= $i ?>', 'matterDesc<?= $i ?>'], ['client_code', 'initial_code', 'mat_description'], 'matter_code')" <?= $redvc ?>>
								<?php if(($user_option == 'Add' || $user_option == 'Select')) { ?>    
									<i class="fa-solid fa-binoculars icn-vw" style="top:20px;" onclick="showData('matter_code', '<?= $displayId['matter_help_id'] ?>', 'matterCode<?= $i ?>', ['clientCode<?= $i ?>', 'initialCode<?= $i ?>', 'matterDesc<?= $i ?>'], ['client_code', 'initial_code', 'mat_description'], 'matter_code')" title="View" data-toggle="modal" data-target="#lookup"></i>
								<?php } ?>
							</td>
							...
							<td class="border fw-normal"> <input type="text" class="form-control" name="client_code<?= $i?>" id="clientCode<?= $i?>"  value="<?= ($row_1) ? $row_1['client_code'] : '' ?>"  readonly></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="initial_code<?= $i?>" id="initialCode<?= $i?>" value="<?= ($row_1) ? $row_1['initial_code'] : '' ?>"   readonly></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="counsel_fee<?= $i?>" value="<?= ($row_1) ? (($row_1['counsel_fee'] == '0') ? '' : $row_1['counsel_fee']) : ''?>" onBlur="chkCounselFee(this,'<?= $i ?>')" <?= $redvc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="clerk_fee<?= $i?>" value="<?= ($row_1) ? (($row_1['clerk_fee'] == '0') ? '' : $row_1['clerk_fee']) : '' ?>" 	onBlur="chkClerkFee(this,'<?= $i ?>')" <?= $redvc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="peon_fee<?= $i?>" value="<?= ($row_1) ? (($row_1['peon_fee'] == '0') ? '' : $row_1['peon_fee']) : '' ?>" 	onBlur="chkPeonFee(this,'<?= $i ?>')" <?= $redvc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="counsel_fee_recd<?= $i?>" value="<?= ($row_1) ? (($row_1['counsel_fee_recd'] == '0') ? '' : $row_1['counsel_fee_recd']) : ''?>" onBlur="chkCounselFeeRecd(this,'<?= $i ?>')" <?= $redkc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="clerk_fee_recd<?= $i?>" value="<?= ($row_1) ? (($row_1['clerk_fee_recd'] == '0') ? '' : $row_1['clerk_fee_recd']) : '' ?>" onBlur="chkClerkFeeRecd(this,'<?= $i ?>')" <?= $redkc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="peon_fee_recd<?= $i?>" value="<?= ($row_1) ? (($row_1['peon_fee_recd'] == '0') ? '' : $row_1['peon_fee_recd']) : '' ?>" onBlur="chkPeonFeeRecd(this,'<?= $i ?>')"  <?= $redkc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="instrument_no<?= $i?>" value="<?= ($row_1) ? $row_1['instrument_no'] : '' ?>" <?= $redkc ?>></td>
							<td class="border fw-normal"> <input type="text" class="form-control" name="instrument_date<?= $i?>" value="<?= ($row_1) ? date_conv($row_1['instrument_date']) : '' ?>" onBlur="chkInstrumentDate(this,'<?= $i ?>')" <?= $redkc ?>></td>
		...
		if(document.f1.user_option.value == 'Add' || document.f1.user_option.value == 'Select') { 
			if(flag == 1) {
				if(conditionFlag || total_row == 0) {
					...
					if(user_option == 'Add' || user_option == 'Select') text += `<td id="Ctd${n}" onClick="voucher_delRow(this, ${n})" class="border fw-normal"align="center"><input type="hidden" name="voucher_ok_ind${n}" value="Y" readonly="true"><img src="<?= base_url('public/assets/img/SelectRow.png') ?>" class="slctRow" alt="Select"/></td>`;
					...
					text += `
						<td class="border fw-normal"> <input type="text" class="form-control" name="brief_date${n}" value="${(total_row) ? document.f1['brief_date'+m].value : ''}" onBlur="chkBriefDate(this,${n})" tabindex="${x1}"></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="memo_no${n}" tabindex="${x2}"></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="memo_date${n}" value="${(total_row) ? document.f1['memo_date'+m].value : ''}" onBlur="chkMemoDate(this,${n})" tabindex="${x3}"></td> 
						<td class="border fw-normal position-relative"> 
							<input type="text" class="form-control" name="matter_code${n}" id="matterCode${n}" onBlur="chkMatterCode(this,${n})" onKeyUp="getMatterCode(this,${n})" tabindex="${x4}" oninput="this.value = this.value.toUpperCase()" onchange="fetchData(this, 'matter_code', ['clientCode${n}', 'initialCode${n}', 'matterDesc${n}'], ['client_code', 'initial_code', 'mat_description'], 'matter_code')">
							<i class="fa-solid fa-binoculars icn-vw" style="top:20px;" onclick="showData('matter_code', '<?= $displayId['matter_help_id'] ?>', 'matterCode${n}', ['clientCode${n}', 'initialCode${n}', 'matterDesc${n}'], ['client_code', 'initial_code', 'mat_description'], 'matter_code')" title="View" data-toggle="modal" data-target="#lookup"></i>
						</td>
						...
						<td class="border fw-normal"> <input type="text" class="form-control" name="initial_code${n}" id="initialCode${n}" readonly="true" ></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="counsel_fee${n}" onBlur="chkCounselFee(this,${n})" tabindex="${x5}"></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="clerk_fee${n}" onBlur="chkClerkFee(this,${n})" tabindex="${x6}"></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="peon_fee${n}" onBlur="chkPeonFee(this,${n})" tabindex="${x7}"></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="counsel_fee_recd${n}" onBlur="chkCounselFeeRecd(this,${n})" tabindex="${x8}" <?= $redkc ?> ></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="clerk_fee_recd${n}" onBlur="chkClerkFeeRecd(this,${n})" tabindex="${x9}" <?= $redkc ?> ></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="peon_fee_recd${n}" onBlur="chkPeonFeeRecd(this,${n})" tabindex="${x10}" <?= $redkc ?> ></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="instrument_no${n}" tabindex="${x11}" <?= $redkc ?> ></td> 
						<td class="border fw-normal"> <input type="text" class="form-control" name="instrument_date${n}" value="${(total_row) ? document.f1['instrument_date'+m].value : ''}" onBlur="chkInstrumentDate(this,${n})" tabindex="${x12}" <?= $redkc ?> ></td> 
						<td class="border fw-normal TbladdBtn"  align="center"><input type="button" name="Add_row${n}" value="+" title="Add Row" onClick="addNewRow(this,${n},${x})" tabindex="${x}"></td>  
					</tr>`;
