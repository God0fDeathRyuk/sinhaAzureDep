...
<?php if((!isset($trans_qry))) { ?> 
    <main id="main" class="main">
        ...
        <div class="pagetitle">
        <h1>List of Advance (Payment)</h1>
        </div><!-- End Page Title -->

        <form action="" method="post" id="advancesPaid" name="advancesPaid" onsubmit="setValue(event)">
                ...
                <div class="col-md-12 mt-2">
                    <div class="frms-sec d-inline-block w-100 bg-white p-3">
                        ...
                        <div class="col-md-3 float-start px-2 mb-1">
                            ...
                            <select class="form-select w-100 float-start" name="payee_type" id="payeeType" onchange="cleanData(this, 'payeeCode', '%&_', 'payeeCodeLookup')" required >
                        ...
                        <div class="col-md-4 float-start px-2 mb-1 position-relative">
                            <label class="d-inline-block w-100 mb-1 lbl-mn">Payee Code</label>					
                            <input type="text" class="form-control w-100 float-start" name="payee_code" id="payeeCode" oninput="this.value = this.value.toUpperCase()" onchange="fetchData(this, 'payee_payer_code', ['payeeName'], ['payee_payer_name'], 'advance_payee', 'advance_type=P&payee_type=@payeeType')" />
                            <i class="fa fa-binoculars icn-vw d-none" aria-hidden="true" id="payeeCodeLookup" onclick="showData('payee_payer_code', 'display_id=<?= $displayId['payee_help_id'] ?>&advance_type=P&payee_type=@payeeType', 'payeeCode', ['payeeName'], ['payee_payer_name'], 'advance_payee')" title="View" data-toggle="modal" data-target="#lookup"></i>
                        </div>
                        <div class="col-md-8 float-start px-2 mb-1">
                            <label class="d-inline-block w-100 mb-1 lbl-mn">Payee Name</label>					
            ...
            <?php 
                        ...
                        <tr>
                            <td colspan="9">    
                                <table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                    <td height="20" class="report_label_text">&nbsp;Payee Name</td>
                ...
                ?>
                                        <tr class="fs-14 border-0">
                                            <td height="22" align="left"  class="p-2" colspan="9">&nbsp;<b><?php echo strtoupper($ppaynm) ?></b></td> 
                                        </tr>
                <?php
                            $ppayind = 'N' ;
                            $lineno  = $lineno + 1 ; 
                ...
                ?>
                                    <tr class="fs-14 border-0">
                                        ...
                                        <td height="16" align="left"  class="p-2">&nbsp;<?php if ($report_row['instrument_dt'] != '0000-00-00') { echo date_conv($report_row['instrument_dt'],'-') ; } else { echo '&nbsp;' ; } ?></td> 
                ...
                ?>                   
                                    ...
                                    <tr class="fs-14 border-0">
                                        <td height="18" align="right" class="p-2" colspan="6" style="background-color: #fdfcc6;">&nbsp;<b>GRAND TOTAL</b>&nbsp;</td>
                                        <td height="18" align="right"  class="p-2" style="background-color: #fdfcc6;"><b><?php if ($tgrsamt > 0) { echo number_format($tgrsamt,2,'.','') ; } else { echo '&nbsp;' ; } ?></b>&nbsp;</td>
                                        <td height="18" align="right"  class="p-2" style="background-color: #fdfcc6;"><b><?php if ($ttaxamt > 0) { echo number_format($ttaxamt,2,'.','') ; } else { echo '&nbsp;' ; } ?></b>&nbsp;</td>
                                        <td height="18" align="right"  class="p-2" style="background-color: #fdfcc6;"><b><?php if ($tnetamt > 0) { echo number_format($tnetamt,2,'.','') ; } else { echo '&nbsp;' ; } ?></b>&nbsp;</td>
                                    </tr>
                            </table>
                            </td>
                        </tr>
...
<script>
   function setValue(e) {
        ...
        if (document.advancesPaid.start_date.value.substring(6,10)+document.advancesPaid.start_date.value.substring(3,5)+document.advancesPaid.start_date.value.substring(0,2) > today_date) {
            Swal.fire({ text: 'Period Start Date must be less than equal to Current Date !!!' }).then((result) => { setTimeout(() => {document.advancesPaid.start_date.focus()}, 500) });
            return false;
        }
        else if (document.advancesPaid.end_date.value.substring(6,10)+document.advancesPaid.end_date.value.substring(3,5)+document.advancesPaid.end_date.value.substring(0,2) > today_date) {
            Swal.fire({ text: 'Period End Date must be less than or equal to Current Date !!!' }).then((result) => { setTimeout(() => {document.advancesPaid.end_date.focus()}, 500) });
            return false;
        }
        else if (document.advancesPaid.start_date.value.substring(6,10)+document.advancesPaid.start_date.value.substring(3,5)+document.advancesPaid.start_date.value.substring(0,2)>document.advancesPaid.end_date.value.substring(6,10)+document.advancesPaid.end_date.value.substring(3,5)+document.advancesPaid.end_date.value.substring(0,2)) {
            Swal.fire({ text: 'Period End Date must be less than Period Start Date !!!' }).then((result) => { setTimeout(() => {document.advancesPaid.end_date.focus()}, 500) });
            return false;
        }
        document.advancesPaid.submit();
    } 
</script>
<?= $this->endSection() ?>