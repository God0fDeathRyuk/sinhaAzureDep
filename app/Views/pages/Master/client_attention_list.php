                  ...
                  //  echo '<pre>';print_r($data["actionLinks"]); echo '</pre>';die;
                    $accessPermission=explode(",", $session->Accpermission); 
                      ...
                      if($permission_name!='ADD' && $permission_name!='LIST'){ ?>
                        <a href="/sinhaco/master/client-attention-addedit/<?php echo strtolower($value['permission_name']) ?>?display_id=<?php echo $params['displayId']; ?>&menu_id=<?php echo $params['menuId']; ?>&uid=<?php echo $row['attention_code']; ?>" class="me-1" title="<?= $value['permission_name'] ?>" <?= (in_array($_REQUEST['menu_id'], $data["detail_linkparam"])) ? 'd-block float-start' : 'd-none'  ?>><i class="<?= $value['icon'] ?> edit" ></i></a>
                     <?php } }}}
                        ?>
                    </td>
                </tr>
