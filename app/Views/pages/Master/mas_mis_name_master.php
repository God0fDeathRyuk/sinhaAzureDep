...
<div class="pagetitle d-block float-start col-md-12">
      <h1>Name Master [<?php echo strtoupper($option) ?>]</h1>      
    </div><!-- End Page Title -->
    <form action="<?php if($option == 'Add'){ echo "/sinhaco/master/mas-mis-name-master?user_option=Add";} if($option == 'Edit'){ echo '/sinhaco/master/mas-mis-name-master?user_option=Edit';}if($option == 'Delete'){ echo '/sinhaco/master/mas-mis-name-master?user_option=Delete';}?>" method="post" >
		  ...
		  <div class="col-md-12 mt-3">
			<div class="frms-sec d-inline-block w-100 bg-white p-3">
				<div class="frms-sec-insde d-block float-start col-md-4 px-2 mb-4 <?php if($option == 'Add'){echo 'd-none'; }else{ echo 'd-block'; }?>">
					...
					<input type="text" class="form-control w-100" placeholder="Code" value="<?= ($option!='Add')?$data['ceo_code']:'' ?>" />
				</div>
				<div class="frms-sec-insde d-block float-start col-md-8 px-2 mb-4">
					...
					<input type="hidden" class="form-control w-100" placeholder="Code" name="ceo_code" id="ceo_code" value="<?= ($option!='Add')?$data['ceo_code']:'' ?>" required/>
					<input type="text" class="form-control w-100" placeholder="name" name="name_desc" id="name_desc" value="<?= ($option!='Add')?$data['name_desc']:'' ?>" required onkeyup="javascript:(this.value=this.value.toUpperCase())"  <?php if ($option == 'Add') {?>	onblur="duplicate_code_check(this.value,'<?php echo $option;?>','getnameNo')" <?php }?> <?php echo $redokadd;?>/>
				</div>			
			</div>		
		  </div>
