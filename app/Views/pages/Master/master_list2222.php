                ...
                <?php foreach ($row as $td) { 
                  ...
                  //  echo '<pre>';print_r($data["actionLinks"]); echo '</pre>';die;
                    $accessPermission=explode(",", $session->Accpermission); 
                      ...
                      <a href="/master/account-head-master/<?php echo strtolower($value['permission_name']) ?>?display_id=<?php echo $params['displayId']; ?>&menu_id=<?php echo $params['menuId']; ?>&uid=<?php echo $row['main_ac_code']; ?>" class="me-1" title="<?= $value['permission_name'] ?>" <?= (in_array($_REQUEST['menu_id'], $data["detail_linkparam"])) ? 'd-block float-start' : 'd-none'  ?>><i class="<?= $value['icon'] ?> edit" ></i></a>
                        
                     <?php } }}}
                        ?>
                     <a href=""  class="me-1" title="EDIT"><i class="fa-solid fa-list-ul edit"></i></a>
                    </td>
                </tr>
                <?php }
            } else { ?>
