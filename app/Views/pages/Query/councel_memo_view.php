			...
			<div class="frms-sec d-inline-block w-100 bg-white p-3 pt-0 position-relative">
				<table class="table table-bordered tblePdngsml mt-3">
					...
					<tr class="fs-14">
						<td>Period</td>
						<td>
							<span class="d-block float-start me-1 mt-2"><?= $end_date_ymd; ?></span>
				...
				<table class="table table-bordered tblePdngsml mt-3">
					...
					<tr class="fs-14">
						...
						<td class="text-end"><span><?= $value['os_amount'] ?></span></td>
					</tr>
                    <?php $tosamt = $tosamt + $value['os_amount'] ; } ?>
					<tr class="fs-14">
						<td class="text-end bgblue" colspan="6">Total</td>
						<td class="text-end bgblue"><?php if($tosamt == 0.00) { echo '&nbsp;'; } else { echo number_format($tosamt,2,'.',''); } ?></td>
					</tr>					
				</table>
			</div>
