...
<?php endif;?>
<div class="pagetitle">
      <h1>Payments Made To Employee</h1>
    </div><!-- End Page Title -->
		...
		<div class="col-md-12 mt-2">
			<div class="frms-sec d-inline-block w-100 bg-white p-3">
				...
				<div class="col-md-3 float-start px-2 mb-1">
                    ...
                    <option value="U">Consultant</option>
					</select>
				</div>
				<div class="col-md-2 float-start px-2 mb-1">
                <label class="d-inline-block w-100 mb-1 lbl-mn">&nbsp;</label>			
					<input type="text" class="form-control w-100 float-start" name="payee_payer_code" id="payee_payer_code" required readonly/>
				</div>
				<div class="col-md-5 float-start px-1 mb-1 mt-2 position-relative">				
					<input type="text" class="form-control w-100 float-start" name="payee_payer_name" id="payee_payer_name" value="<?= ($options!='')?$data2[0]['payee_payer_name']:'' ?>" required readonly />
	...
	<?php if ($options!=''){foreach ($data2 as $key => $value) {?>
		<form method="post" action="/sinhaco/query/consltn-voucher-view" name="qry_vou_frm<?= $key ?>" id="qry_vou_frm<?= $key ?>" >
