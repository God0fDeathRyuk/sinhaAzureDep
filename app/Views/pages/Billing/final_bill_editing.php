...
<div class="pagetitle col-md-12 float-start border-bottom pb-1">
  <h1>Final Bill Editing</h1>
</div>

<form action="" method="post" id="billEditingFinal">
<input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>">
	<div class="row">
		...
		<div class="inpt-grp col-md-4 pe-0 position-relative">
			...
			<i class="fa-solid fa-eye inpt-vw" onclick="showData('serial_no', '<?= $displayId['billsrl_am_help_id'] ?>', 'SerialNo', ['matterCode', 'matterDesc', 'clientCode', 'clientName'], ['matter_code', 'matter_desc', 'client_code', 'client_name'], 'bill_serial_code')" data-toggle="modal" data-target="#lookup"></i>
            <?php } ?>
		</div>
		<div class="inpt-grp col-md-4 pe-0">
		...
		<div class="inpt-grp col-md-12 pe-0 mt-1">
		...
		<div class="w-100 float-start text-start mt-4 top-btn-fld">
			<button type="submit" class="btn btn-primary cstmBtn mt-0 me-2" onclick="formOption('/billing/final-bill-editing/', 'edit', 'billEditingFinal')">Edit</button>
			<button type="submit" class="btn btn-primary cstmBtn mt-0 me-2" onclick="formOption('/billing/final-bill-editing/', 'view', 'billEditingFinal')">View</button>
...
<?php } else { ?>
	<form action="" method="post">
		...
		<div class="frms-sec d-inline-block w-100 bg-white p-3">
			<div class="d-inline-block w-100">
				<div class="frms-sec-insde d-block float-start col-md-4 px-2 mb-4">
					<label class="d-inline-block w-100 mb-2 lbl-mn">Bill Year</label>
					<input type="text" class="form-control" name="bill_year" value="<?php echo $params['bill_year']; ?>" readonly/>
				</div>
				<div class="frms-sec-insde d-block float-start col-md-4 px-2 mb-4 position-relative">
					<label class="d-inline-block w-100 mb-2 lbl-mn">Bill No</label>
					<input type="text" class="form-control" name="bill_no" id="billNo" value="<?php echo $params['bill_no']; ?>" onchange="showHistoryBtn(this, 'billNoHistory')" readonly/>
					<i class="fa-solid fa-clock inpt-vw histryIcn" id="billNoHistory" title="View Draft Bill" onclick="showHistory('billing/history/draft-bill', 'serial_no=@billNo&row_count=1')" data-toggle="modal" data-target="#lookup"></i>
				</div>
				<div class="frms-sec-insde d-block float-start col-md-3 px-2 mb-4 position-relative">
					<label class="d-inline-block w-100 mb-2 lbl-mn">Bill Serial</label>
					<input type="text" class="form-control" name="serial_no" id="billSerialNo" value="<?php echo $row['serial_no']; ?>" onchange="showHistoryBtn(this, 'draftBillHistory')" readonly />
					<i class="fa-solid fa-clock inpt-vw histryIcn" id="draftBillHistory" title="View Draft Bill" onclick="showHistory('billing/history/draft-bill', 'serial_no=@billSerialNo&row_count=1')" data-toggle="modal" data-target="#lookup"></i>
				</div>
				<div class="frms-sec-insde d-block float-start col-md-6 ps-2 mb-4">
					...
					<input class="form-control float-start w-48 ms-2 set-date datepicker" id="start_date" type="text" name="start_date" value="<?php echo date_conv($row['start_date']); ?>" readonly>
					<span class="float-start mt-2 ms-2">-</span>
					<input  class="form-control float-start w-42 ms-2 set-date datepicker" id="end_date" type="text" name="end_date" value="<?php echo date_conv($row['end_date']); ?>" readonly>
				</div>
			...
			<div class="col-md-6 float-start px-2 position-relative mb-3">
				...
				<input type="text" class="form-control w-35 float-start" name="matter_code" value="<?php echo $row['matter_code'];?>" readonly/>
				<input type="text" class="form-control w-63 ms-2 float-start" name="matter_desc" value="<?php echo $params['maatter_description'];?>" readonly/>
			</div>
			<div class="col-md-6 float-start px-2 position-relative mb-3">
				<label class="d-inline-block w-100 mb-2 lbl-mn">Client Code</label>
			...
			<div class="col-md-12 float-start px-2 mb-3">
				...
				<textarea type="text" class="form-control" rows="2" name="other_case_desc" readonly><?= $row['other_case_desc'] ?></textarea>
			</div>
			<div class="col-md-6 float-start px-2 mb-3">
				<label class="d-inline-block w-100 mb-2 lbl-mn">Subject</label>
				<textarea type="text" class="form-control" rows="2" name="subject_desc" <?= $permission ?>><?= strtoupper($row['subject_desc']) ?></textarea>
			</div>
			<div class="col-md-6 float-start px-2 mb-3">
				<label class="d-inline-block w-100 mb-2 lbl-mn">Ref No</label>
				<textarea type="text" class="form-control" rows="2" name="reference_desc" <?= $permission ?>><?= $row['reference_desc'] ?></textarea>
			</div>		
			...
			<div class="col-md-2 float-start px-2 mb-3">
				...
				<select class="form-select" name="direct_counsel_ind" <?= $permission ?>>
					<option value="N" <?php if($row['direct_counsel_ind'] != 'Y') { echo 'selected' ; } ?>>No</option>
					<option value="Y" <?php if($row['direct_counsel_ind'] == 'Y') { echo 'selected' ; } ?>>Yes</option>
				</select>	
			</div>
			...
			<div class="col-md-2 float-start px-2 mb-3">
				...
				<select class="form-select" name="court_fee_bill_ind" <?= $permission ?>>
					<option value="N" <?php if($row['court_fee_bill_ind'] != 'Y') { echo 'selected' ; } ?>>No</option>
					<option value="Y" <?php if($row['court_fee_bill_ind'] == 'Y') { echo 'selected' ; } ?>>Yes</option>
				</select>
			</div>
			<div class="col-md-2 float-start px-2 mb-3">
				<label class="d-inline-block w-100 mb-2 lbl-mn">No Free Bill</label>
				<select class="form-select" name="no_fee_bill_ind" <?= $permission ?>>
					<option value="N" <?php if($row['no_fee_bill_ind'] != 'Y') { echo 'selected' ; } ?>>No</option>
					<option value="Y" <?php if($row['no_fee_bill_ind'] == 'Y') { echo 'selected' ; } ?>>Yes</option>
				</select>
			</div>
					...
					<?php foreach($qry2 as $key => $row) { ?>
						<tr>
							<td id="Ctd<?php echo $key+1;?>" onClick="<?php if($option == 'edit') { ?>inpocket_delRow(this, <?php echo $key+1;?>)<?php } ?>">
								<input type="hidden" name="inp_ok_ind<?php echo $key+1?>" id="inp_ok_ind<?php echo $key+1?>" value="Y" onClick="<?php if($option == 'edit') { ?>inpocket_delRow(<?php echo $key+1;?>)<?php } ?>" <?= $permission ?>/>
							...
							<td>
								<input style="width:120px;" class="form-control" type="text" name="activity_date<?php echo $key+1?>" id="activity_date<?php echo $key+1?>" value="<?= date_conv($row['activity_date'])?>" onblur="<?php if($option == 'edit') { ?>chkActivity(this,<?php echo $key+1?>)<?php } ?>" <?= $permission ?>/>
							</td>
							<td>
							...
							<td>
								<input style="width:125px;" class="form-control" type="text" name="io_ind<?php echo $key+1?>" id="io_ind<?php echo $key+1?>" value="<?= ($row['io_ind'] == 'O') ? $row['io_ind'] = 'C' : $row['io_ind'] ?>" onkeyup="<?php if($option == 'edit') { ?>changeCase(this,<?php echo $key+1; ?>);calc_service_tax(<?php echo $key+1;?>,'<?php echo $params['tax_per']?>','Grid1');<?php } ?>" <?= $permission ?>/>
							</td>
							<td>
								<input style="width:125px;" class="form-control" type="text" name="billed_amount<?php echo $key+1;?>" id="billed_amount<?php echo $key+1;?>" onkeypress="return validnumbercheck(event);" onblur="<?php if($option == 'edit') { ?>amtValidate(this,<?php echo $key+1; ?>,'Grid1'); calc_service_tax('<?php echo $key+1?>','<?php echo $params['tax_per']?>','Grid1')<?php } ?>" value="<?= number_format($row['billed_amount'],2,'.','')?>" <?= $permission ?>/>
							</td>
			...
			<div class="d-inline-block w-100 mt-2 scrlTblMd">
				<table class="table table-bordered tblhdClr">
					...
					<?php foreach($qry3 as $key_1 => $row_1) { ?>
						<tr>
							<td style="width:20px;" onClick="<?php if($option=='edit'){ ?>outpocket_delRow(this, <?php echo $key_1+1;?>)<?php }?>">
							...
							<td class="brkwrd optnNone" style="width:120px;">
								<input class="form-control w-100 ckeditor" name="details<?php echo $key_1+1?>" id="details<?php echo $key_1+1?>" value="<?= stripslashes($row_1['activity_desc'])?>" <?= $permission ?>/> 
							</td>
							<td style="width:85px;"> 
								<input class="w-100 text-left" class="form-control" name="amount<?php echo $key_1+1;?>" id="amount<?php echo $key_1+1;?>" value="<?= number_format($row_1['billed_amount'],2,'.','')?>" onBlur="<?php if($option == 'edit') { ?>amtValidate(this,<?php echo $key_1+1; ?>,'Grid2'); calc_service_tax('<?php echo $key_1+1;?>','<?php echo $params['tax_per']?>','Grid2')<?php } ?>" <?= $permission ?>/>
							</td>
							<td style="width:75px;">
							...
							<td style="width:75px;">
								<input class="w-100" type="checkbox" class=""  name="printer<?php echo $key_1+1?>" id="printer<?php echo $key_1+1?>" value="Y" onClick="myCheckPrintInd('<?php echo $key_1+1;?>','Grid2')" <?php if($row_1['printer_ind'] == 'Y') { echo 'checked';}?> <?= $permission ?>/>
							</td>
							<td style="width:75px;">
		...
		<input type="hidden" name="serial_no" id="serial_no" value="<?php echo $params['serial_no'];?>">
		<input type="hidden" name="ref_bill_serial_no" id="ref_bill_serial_no" value="<?php echo $params['ref_bill_serial_no'];?>">
	</form>
		...
		function addRowInQry2(e, flag = false, params) {
			...
			let td = `
				<tr>
					...
					<td>
						<input style="width:75px;" class="form-control" type="text" name="billed_amount${key}" id="billed_amount${key}" onkeypress="return validnumbercheck(event);" onblur="<?php if ($option == 'edit') { ?>amtValidate(this,${key},'Grid1'); calc_service_tax('${key}', '<?php echo $params['tax_per'] ?>', 'Grid1')<?php } ?>" value="" />
					</td>
					<td>
		...
		function addRowInQry3(e, flag = false, params) {
			...
			let td = `
				<tr>
					...
					<td style="width:85px;">
						<input class="w-100 text-left form-control" name="amount${key}" id="amount${key}" value="" onBlur="<?php if ($option == 'edit') { ?>amtValidate(this, ${key}, 'Grid2'); calc_service_tax('${key}','<?php echo $params['tax_per'] ?>','Grid2')<?php } ?>" />
					</td>
					<td style="width:75px;">
